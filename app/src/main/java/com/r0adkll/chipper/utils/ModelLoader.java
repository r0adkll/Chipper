...
public class ModelLoader<T extends Model> extends AsyncTaskLoader<List<T>> {
     ...
     * @param updateOnRelationshipChanges if true, loader will updated when tables related to the one detected are changed
     */
    public ModelLoader(Context context, Class<T> clazz, ResultQueryBase from,
    ...
    public List<T> loadInBackground() {
        ...
        if (mQuery == null) {
            results = Select.from(mClass).fetch();
        } else {
            results = mQuery.fetch();
        }
    ...
    protected void onStartLoading() {
        ...
        if (mContentObserver == null) {
            ...
            mContentResolver.registerContentObserver(OllieProvider.createUri(mClass, null), true,
                    mContentObserver);
            ...
            if (mUpdateOnRelationshipChanges) {
                ...
                for (Field field : fields) {
                    ...
                    if (Model.class.isAssignableFrom(clazz)) {
                        mContentResolver.registerContentObserver(
                                OllieProvider.createUri(clazz.asSubclass(Model.class), null),
                ...
                for (Method method : methods) {
                    ...
                    if (Model.class.isAssignableFrom(returnClass)) {
                        mContentResolver.registerContentObserver(
                                OllieProvider.createUri(returnClass.asSubclass(Model.class), null),
                                true, mContentObserver);
                    } else {
